...
static void a2s_declinit(Buffer *b, Vector *initlist, char * (*nodefunc)(Node *)) {
    for (int i = 0; i < vec_len(initlist); i++) {
        if (i > 0)
            buf_printf(b, " ");
        Node *init = vec_get(initlist, i);
		buf_printf(b, "%s", nodefunc(init));
    }
}

static void literal2s(Buffer * b, Node * node)
{
	switch (node->ty->kind) {
	case KIND_CHAR:
		if (node->ival == '\n')      buf_printf(b, "'\n'");
...
static void do_a2s2(Buffer * b, Node * node)
{
	if (!node)
	{
		buf_printf(b, "NULL");
	...
	case AST_LVAR:
		...
		if (node->lvarinit) {
			buf_printf(b, "(");
			a2s_declinit(b, node->lvarinit, a2s2);
			buf_printf(b, ")");
		}
		break;
	case AST_GVAR:
	...
	case AST_FUNCPTR_CALL: {
		buf_printf(b, "(%s)%s(", c2s(node->ty),
			node->kind == AST_FUNCALL ? node->fname : a2s2(node));
		for (int i = 0; i < vec_len(node->args); i++) {
			if (i > 0)
				buf_printf(b, ",");
			buf_printf(b, "%s", a2s2(vec_get(node->args, i)));
		}
		buf_printf(b, ")");
		break;
	}
	...
	case AST_FUNC: {
		...
		for (int i = 0; i < vec_len(node->params); i++) {
			...
			buf_printf(b, "%s %s", c2s(param->ty), a2s2(param));
		}
		buf_printf(b, ")");
		do_a2s2(b, node->body);
		break;
	...
	case AST_DECL:
		...
		if (node->declinit) {
			buf_printf(b, " ");
			a2s_declinit(b, node->declinit, a2s2);
		}
		buf_printf(b, ")\n");
		break;
	case AST_INIT:
		buf_printf(b, "%s@%d", a2s2(node->initval), node->initoff, c2s(node->totype));
		break;
	case AST_CONV:
		buf_printf(b, "(conv %s=>%s)", a2s2(node->operand), c2s(node->ty));
		break;
	case AST_IF:
		buf_printf(b, "(if %s %s",
			a2s2(node->cond),
	...
	case AST_TERNARY:
		...
		break;
	case AST_RETURN:
		buf_printf(b, "(return %s)", a2s2(node->retval));
		break;
	case AST_COMPOUND_STMT: {
		buf_printf(b, "{\n");
		for (int i = 0; i < vec_len(node->stmts); i++) {
			do_a2s2(b, vec_get(node->stmts, i));
			buf_printf(b, ";\n");
		}
		buf_printf(b, "\n}");
		break;
	...
	case OP_LABEL_ADDR:
		...
		break;
	default: {
		char *left = a2s2(node->left);
		char *right = a2s2(node->right);
		if (node->kind == OP_EQ)
			buf_printf(b, "(== ");
		else
...
static void do_a2s(Buffer *b, Node *node) {
    ...
    case AST_LVAR:
        ...
        if (node->lvarinit) {
            buf_printf(b, "(");
            a2s_declinit(b, node->lvarinit, a2s);
            buf_printf(b, ")");
        }
        break;
    case AST_GVAR:
    ...
    case AST_DECL:
        ...
        if (node->declinit) {
            buf_printf(b, " ");
            a2s_declinit(b, node->declinit, a2s);
        }
        buf_printf(b, ")");
        break;
    case AST_INIT:
